...
#define kXHVoiceRecordDoingString @"手指上滑，取消发送"
#define kXHVoiceRecordCancelingString @"松开手指，取消发送"
#define kXHVoiceRecordErrorString @"录音错误，请重试"
...
- (void)startRecordingHUDAtView:(UIView *)view {
    if (nil != view && ![view isEqual:self.superview]) {
...
- (void)didCanceling {
    ...
    self.remindLabel.text = kXHVoiceRecordCancelingString;
}
...
- (void)showError
{
    ...
    self.remindLabel.text = kXHVoiceRecordErrorString;
}
...
- (void)setup {
    ...
    if (!_remindLabel) {
        ...
        remindLabel.text = kXHVoiceRecordDoingString;
